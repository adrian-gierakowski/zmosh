...
pub const Gateway = struct {
    ...
    pub fn init(
        ...
        const socket_path = try socket.getSocketPath(alloc, socket_dir, session_name);
        ...
        const flags = try posix.fcntl(unix_fd, posix.F.GETFL, 0);
        _ = try posix.fcntl(unix_fd, posix.F.SETFL, flags | posix.SOCK.NONBLOCK);
